...
import lab.cocelo.tasks.manager_based.locomotion.velocity.flamingo_pro_env.rough_env.stand_drive.drive_rewards as mdp_drive
from lab.cocelo.tasks.manager_based.locomotion.velocity.flamingo_pro_env.velocity_env_cfg import (
    LocomotionVelocityRoughEnvCfg,
    CurriculumCfg,
)

from lab.cocelo.assets.flamingo.flamingo_pro_v3 import FLAMINGO_CFG  # isort: skip
...
class FlamingoRewardsCfg():
    ...
    backward_falloff_on_step = RewTerm(
        func=mdp_drive.backward_falloff_on_step,
        weight=-6.0,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=[".*_wheel_static_link"]),
            "command_name": "base_velocity",
            "height_diff_threshold": 0.04,
            "backward_vel_threshold": 0.02,
        },
    )
    ...
    dof_acc_l2 = RewTerm(func=mdp.joint_acc_l2, weight=-5.0e-7)  # default: -2.5e-7
    action_rate_l2 = RewTerm(func=mdp.action_rate_l2, weight=-0.02)  # default: -0.01
    stay_alive = RewTerm(func=mdp.stay_alive, weight=2.0)  # default: 1.0
